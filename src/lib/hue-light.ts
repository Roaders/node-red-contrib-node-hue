...
LightItem.prototype.updateInfo = function updateInfo(hueInfo) {
  ...
  isUpdated = isUpdated || !_.isEqual(this.state, newState);
  if (isUpdated) {
    this.state = newState;
    this.info.capability = calculateCapabilties(this.state);
  }
...
LightItem.prototype.parseColorRGB = function parseColorRGB(input, output) {
  ...
  if (changed & LightCapability.XY) {
    ...
    if (this.info.capability & LightCapability.XY) {
      output.colormode = 'xy';
      output.xy = [
        limitValue(Math.round(10000*xy[0])/10000, 0, 1),
        limitValue(Math.round(10000*xy[1])/10000, 0, 1)
      ];
    }
    // Convert to HSV color
    else if (this.info.capability & LightCapability.HS) {
      output.colormode = 'hs';
      output.hue = limitValue(Math.round((hsv[0] * 0xFFFF) / 359), 0, 0xFFFF);
...
LightItem.prototype.setColor = function setColor(input) {
  ...
  if (this.state.on) {
    ...
    // Alert handling
    if (_.isString(input.alert) && input.alert.length) {
    ...
    // Effect handling
    if (_.isString(input.effect) && input.effect.length) {
